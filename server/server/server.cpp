...
void main()
{
	...
	while (true)
	{
		...
		// Get the current time - now we want  a new answer
		if (!strcmp(recvBuff, "how are you?"))
		{
			strcpy(sendBuff, "fine!");
		}
		else if (!strcmp(recvBuff, "how is the weather?"))
		{
			strcpy(sendBuff, "good!");
		}
		else if (!strcmp(recvBuff, "what is the time?"))
		{
			time_t timer;
			time(&timer);
